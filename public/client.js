...
const app = Vue.createApp({
  ...
  methods: {
      ...
      fetch(`/api/getFollowings?next_token=${next_token}`)
        ...
        .then((data) => {
          if ("error" in data) {
            ...
            console.log('followings: ', data.accounts[0].username);
    ...
    async rsslayCall(element) {
          let data = {}
          const response = await fetch(element.rsslayURL, {
            method: 'POST',
            mode: 'cors',
            cache: 'no-cache',
    ...
    async getNostrProfile(followingSnippet) {
        followingSnippet.forEach(element => {
          const rsslayURL = "https://rsslay.nostr.moe/api/feed?url=https://nitter.moomoo.me/" + element.username + "/rss";
          element.rsslayURL = rsslayURL;
          this.rsslayCall(element);
        });
    },    
